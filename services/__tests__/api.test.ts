...
describe('updateReviewGenre', () => {
  ...
  test('falls back to materialize on 404', async () => {
    ...
    const materializeResp = { review: { id: 'arch1' }, genre: { id: 'g1', name: 'n' }, materialized: 'updated' };
  ...
  test('returns response when patch succeeds', async () => {
    const patchResp = { ok: true, json: async () => ({ review: { id: 'r1' }, genre: { id: 'g1', name: 'n' } }) } as any;
